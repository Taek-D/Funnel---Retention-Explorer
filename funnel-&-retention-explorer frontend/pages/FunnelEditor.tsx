    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center bg-gradient-to-r from-primary/10 to-transparent p-4 rounded-xl border border-primary/20">
        ...
        <div className="flex gap-3">
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${detectedType === 'ecommerce' ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-slate-400 hover:text-white border border-white/10'}`}
          >
            E-commerce
          </button>
          <button
            onClick={() => applyTemplate('subscription')}
            className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${detectedType === 'subscription' ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-slate-400 hover:text-white border border-white/10'}`}
          >
            Subscription
          </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 flex flex-col gap-4">
          <div className="rounded-xl border border-white/10 bg-surface p-5 flex flex-col">
            <div className="flex items-center justify-between border-b border-white/10 pb-4 mb-4">
              <h3 className="text-white text-sm font-bold uppercase tracking-wider">Funnel Steps</h3>
              <span className="text-[10px] text-slate-600">{funnelSteps.length} steps</span>
            </div>

            <div className="flex flex-col gap-2 overflow-y-auto max-h-[500px]">
              {funnelSteps.map((step, i) => (
                <div key={i} className="group flex items-center gap-2 p-3 rounded-lg border border-white/10 bg-background hover:border-primary/50 transition-colors">
                  <div className="bg-primary/20 text-primary w-5 h-5 rounded flex items-center justify-center text-[10px] font-bold shrink-0">{i + 1}</div>
                  <select
                    className="flex-1 bg-transparent text-white text-sm border-none outline-none"
                    value={step}
                    onChange={(e) => updateStep(i, e.target.value)}
                  >
                    <option value="" className="bg-[#151a2d]">이벤트 선택...</option>
                    {uniqueEvents.map(event => (
                      <option key={event} value={event} className="bg-[#151a2d]">{event}</option>
                    ))}
                  </select>
                  {i > 0 && (
              ...
              <button
                onClick={addStep}
                className="mt-2 w-full py-2.5 rounded-lg border border-dashed border-slate-600 text-slate-400 hover:text-white hover:border-primary hover:bg-primary/5 transition-all text-sm font-medium flex items-center justify-center gap-2"
              >
            ...
            <button
              ...
              className="mt-4 w-full py-3 rounded-lg bg-primary hover:bg-primary/90 text-white text-sm font-bold shadow-lg shadow-primary/25 transition-all"
            >
              Calculate Funnel
            </button>
          </div>
        ...
        {/* Preview Area */}
        <div className="lg:col-span-8 rounded-xl border border-white/10 bg-surface p-6 flex flex-col">
          <div className="flex justify-between items-center border-b border-white/10 pb-4 mb-8">
            <div>
              ...
              <p className="text-sm text-slate-400">
                {funnelResults ? `${funnelResults[0]?.users.toLocaleString()} users analyzed` : 'Configure steps and click Calculate'}
              </p>
            </div>
          </div>
              ...
              funnelResults.map((step, i) => {
                ...
                return (
                  <div key={i} className="flex flex-col items-center justify-end h-full flex-1 gap-2 group">
                    <div
                      className="w-full bg-gradient-to-t from-primary/60 to-primary rounded-t-lg relative hover:brightness-110 transition-all"
                    ...
                    <div className="text-center">
                      ...
                      <span className="text-white font-bold text-sm">{step.users.toLocaleString()}</span>
                    </div>
                  </div>
                );
              })
